// Global usings — tất cả file trong Business đều có thể dùng các namespace này mà không cần khai báo lại
global using ShoesShop.Shared;
global using ShoesShop.Shared.Constants;
global using ShoesShop.Shared.Helpers;
global using ShoesShop.Shared.DTOs;
global using ShoesShop.Shared.DTOs.Product;
global using ShoesShop.Shared.DTOs.Auth;
global using ShoesShop.Shared.DTOs.Address;
global using ShoesShop.Shared.DTOs.User;
global using ShoesShop.Shared.DTOs.Admin;
global using ShoesShop.Shared.DTOs.Catalog;
global using ShoesShop.Shared.DTOs.Order;
global using ShoesShop.Shared.DTOs.Voucher;
global using ShoesShop.Shared.DTOs.Review;
global using ShoesShop.Shared.DTOs.Dashboard;
